// ============================================================================
// js/combat.js - GESTION DES ATTAQUES ET DE LA BRÛLURE
// ============================================================================
...
window.handlePlayerAttack = function() {
    ...
    if (player.heroClass === 'Elf') {
        ...
        currentEnemies.forEach(enemy => { 
            if (window.checkCollision(hitBox, enemy) && !enemy.invulnerable) {
                if (enemy.type === 'goblin' && Math.random() < 0.15) { enemy.blockAnimTimer = 45; if (typeof window.spawnParticles === 'function') window.spawnParticles(enemy.x + enemy.size/2, enemy.y + enemy.size/2, '#bdc3c7', 15); } 
                else { enemy.health -= 50; if (typeof window.spawnParticles === 'function') window.spawnParticles(enemy.x + enemy.size/2, enemy.y + enemy.size/2, enemy.color, 15); if (typeof window.triggerShake === 'function') window.triggerShake(5, 8); }
            } 
        });
...
window.updateProjectiles = function() {
    ...
    for (let i = projectiles.length - 1; i >= 0; i--) {
        ...
        if (p.x < bLeft || p.y < wallMargin || p.x > bRight || p.y > canvas.height - wallMargin) { projectiles.splice(i, 1); continue; }
        ...
        for (let j = 0; j < currentEnemies.length; j++) {
            ...
            if (!projectileHit && window.checkCollision(arrowHitbox, enemy)) {
                if (!enemy.invulnerable) {
                    let isBlocked = false;
                    if (enemy.type === 'goblin' && Math.random() < 0.15) { isBlocked = true; enemy.blockAnimTimer = 45; if (typeof window.spawnParticles === 'function') window.spawnParticles(enemy.x + enemy.size/2, enemy.y + enemy.size/2, '#bdc3c7', 15); }
                    if (!isBlocked) { 
                        let dmg = 30; if (player.heroClass === 'Elf') dmg = 60; else if (player.heroClass === 'Mage') dmg = 35;
                        enemy.health -= dmg; 

                        // FIX : LA BRÛLURE CLASSIQUE EST DE RETOUR
                        if (p.isFire) {
