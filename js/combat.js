...
// Déclenché par le clic gauche de la souris (depuis controls.js)
    ...
    if (player.heroClass === 'Elf') {
        if (isUltimateActive) {
            ...
            projectiles.push({ 
                ...
                size: 5, hitTargets: [], angle: angle - spread, isFire: false 
            });
            projectiles.push({ 
                x: player.x + player.size / 2, y: player.y + player.size / 2, 
                vx: Math.cos(angle + spread) * 12, vy: Math.sin(angle + spread) * 12, 
            ...
            projectiles.push({ 
                ...
                size: 5, hitTargets: [], angle: angle, isFire: false 
            }); 
            attackCooldown = 30; 
        }
    } 
        ...
        projectiles.push({ 
            ...
            size: 8, hitTargets: [], angle: angle, isFire: true 
        }); 
        attackCooldown = 35;
    } 
        ...
        projectiles.push({ 
            x: player.x + player.size / 2, y: player.y + player.size / 2, 
            vx: Math.cos(angle) * 10, vy: Math.sin(angle) * 10, 
            size: 6, hitTargets: [], angle: angle, isFire: false, isNecro: true 
        }); 
        attackCooldown = 15;
    } 
...
window.updateItemsAndCrates = function() {
    // RAMASSAGE DES OBJETS AU SOL (Le Point Jaune Fix)
    ...
    // DESTRUCTION DES CAISSES ET APPARITION DU LOOT
    for (let i = 0; i < currentCrates.length; i++) {
        ...
        if (!crate.isBroken && crate.health <= 0) {
            ...
            if (crate.type === 'chest') {
                ...
                // Le butin apparaît devant le coffre (Y + 20) pour ne pas être coincé dedans !
