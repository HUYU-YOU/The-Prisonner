...
window.handlePlayerAttack = function() {
    ...
    if (player.heroClass === 'Elf') {
        ...
        window.projectiles.push({ x: player.x + player.size / 2, y: player.y + player.size / 2, vx: Math.cos(angle) * 22, vy: Math.sin(angle) * 22, size: 8, hitTargets: [], angle: angle, type: 'fire_mage' }); 
        attackCooldown = 25; 
    } 
...
window.updateProjectiles = function() {
    ...
    for (let i = window.enemyProjectiles.length - 1; i >= 0; i--) {
        ...
        if (!fusionAggro && !isElfInvuln && playerInvulnerableTimer <= 0 && typeof window.checkCollision === 'function' && window.checkCollision(player, epHitbox)) {
