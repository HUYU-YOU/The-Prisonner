...
window.loadRoom = function(roomId, entryFace = 'south') {
    ...
    if (roomId === 1) { 
        ...
        currentObstacles.push({ x: wallMargin, y: wallMargin, width: 300, height: canvas.height - wallMargin*2, type: 'hole' });
        if (!worldState.collectedItems['key_room102']) {
            currentItems.push({ id: 'key_room102', type: 'key', x: canvas.width/2, y: canvas.height/2, size: 20, collected: false });
        }
    }
    else if (roomId === 103) { 
        currentDoors = [ 
        ...
        currentObstacles.push({ x: canvas.width/2 - 150, y: 0, width: 300, height: canvas.height, type: 'hole' });
    }
    else if (roomId === 104) { 
    ...
    if (roomId === 999) {
        ...
        if (worldState.enemyStates[roomId]) { 
            ...
            else if (roomId === 3) window.spawnEnemy('goblin', 2, canvas.width - 200, canvas.height/2);
            ...
            else if (roomId === 102) window.spawnEnemy('skeleton', 2, wallMargin + 300, canvas.height/2);
            else if (roomId === 103) { 
                window.spawnEnemy('skeleton', 1, canvas.width - 150, 150); 
                window.spawnEnemy('skeleton', 1, canvas.width - 150, canvas.height - 150); 
            ...
            else if (roomId === 207) { window.spawnEnemy('siren', 1, spawnOppositeX - 60, spawnOppositeY); window.spawnEnemy('siren', 1, spawnOppositeX + 60, spawnOppositeY); window.spawnEnemy('siren', 1, spawnOppositeX, spawnOppositeY + 60); }
            else if (roomId === 208) { window.spawnEnemy('kraken', 1, canvas.width/2 - 90, canvas.height/2 - 90); window.spawnEnemy('siren', 1, canvas.width/2 - 150, canvas.height/2); window.spawnEnemy('siren', 1, canvas.width/2 + 150, canvas.height/2); }
        }
    }
};
