...
window.spawnEnemy = function(type, count, baseX = null, baseY = null) {
    for (let i = 0; i < count; i++) {
        ...
        currentEnemies.push({ 
            ...
            shootCooldown: 0, summonTimer: 180, wobble: Math.random() * Math.PI * 2, timeAlive: 0, phase: 1, 
            invulnerable: false, isBurning: false, burnTicks: 0, burnTimer: 0, slowTimer: 0, isPermanentlySlowed: false, 
            killedBySummon: false, killedByNecro: false, attackAnimTimer: 0, blockAnimTimer: 0, ultiAnimTimer: 0, 
            dashTimer: 180, isDashing: 0
        });
    }
};
...
window.updateEnemies = function() {
    ...
    // 1. DÉPLACEMENT ET COLLISIONS DES ENNEMIS
    ...
    for (let i = currentEnemies.length - 1; i >= 0; i--) {
        if (currentEnemies[i].health <= 0) {
            ...
            // Drop de pièces d'or (30% de chance)
