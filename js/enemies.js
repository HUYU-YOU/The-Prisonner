...
window.spawnEnemy = function(type, count, baseX = null, baseY = null) {
    for (let i = 0; i < count; i++) {
        ...
        else if (type === 'golem') { size = 65; hp = 100; spd = 1.5; col = '#7f8c8d'; } 
        else if (type === 'small_golem') { size = 39; hp = 40; spd = 2.5; col = '#bdc3c7'; } 
...
window.updateEnemies = function() {
    ...
    currentEnemies.forEach((enemy, idx) => {
        ...
        if (enemy.type === 'boulder') {
            ...
            if (window.checkCollision(player, enemy) && playerInvulnerableTimer <= 0 && !isElfInvuln) {
        ...
        if (enemy.type === 'anglerfish') {
            let isAggro = enemy.health < enemy.maxHealth;
            
            if (!isAggro) {
                if (enemy.wanderTimer === undefined || enemy.wanderTimer <= 0) {
                    enemy.wanderAngle = Math.random() * Math.PI * 2;
                    enemy.wanderTimer = 90 + Math.random() * 90;
                }
                enemy.wanderTimer--;
                dx = Math.cos(enemy.wanderAngle) * (dist || 1); 
                dy = Math.sin(enemy.wanderAngle) * (dist || 1);
                enemy.faceAngleTarget = enemy.wanderAngle;
                
                let newSize = enemy.baseSize || 60;
                if (enemy.size !== newSize) {
                    let cx = enemy.x + enemy.size/2; let cy = enemy.y + enemy.size/2;
                    enemy.size = newSize; enemy.x = cx - enemy.size/2; enemy.y = cy - enemy.size/2;
                }
            } else {
                dx = targetObj.x - enemy.x; dy = targetObj.y - enemy.y;
                enemy.faceAngleTarget = Math.atan2(dy, dx);
                
                let newSize = (dist < 150) ? (enemy.baseSize || 60) * 2 : (enemy.baseSize || 60);
                if (enemy.size !== newSize) {
                    let cx = enemy.x + enemy.size/2; let cy = enemy.y + enemy.size/2;
                    enemy.size = newSize; enemy.x = cx - enemy.size/2; enemy.y = cy - enemy.size/2;
                }

                if (dist < (player.size + enemy.size) * 0.75 && playerInvulnerableTimer <= 0 && !isElfInvuln) {
                    playerStats.health = 0;
                    if (typeof window.handlePlayerDeath === 'function') window.handlePlayerDeath();
                }
            }
        ...
        if (enemy.type === 'dragon') {
            if (enemy.phase === 1 && enemy.health <= enemy.maxHealth / 2) { enemy.phase = 2; }
            ...
            if (enemy.shootCooldown <= 0 && !isElfInvuln) {
                ...
                enemy.shootCooldown = 60; enemy.attackAnimTimer = 30; 
            }
        }
        ...
        if (enemy.type === 'troll') {
            ...
            if (enemy.isTelegraphing > 0) {
                ...
                enemy.isDashing--; currentEnemySpeed *= 8.0; 
            }
        }
        ...
        currentEnemies.forEach((otherEnemy, otherIdx) => {
            if (idx !== otherIdx && enemy.type !== 'gargouille' && enemy.type !== 'boulder') {
        ...
        if (typeof currentObstacles !== 'undefined') {
            for (let obs of currentObstacles) {
                if (obs.type === 'hole' && window.checkCollision(enemy, obs)) { 
                    enemy.x = oldEx; hitX = true; 
                    enemy.y += (enemy.y < obs.y + obs.height/2) ? -2 : 2; break; 
                }
            }
        }
        ...
        if (typeof currentObstacles !== 'undefined') {
            for (let obs of currentObstacles) {
                if (obs.type === 'hole' && window.checkCollision(enemy, obs)) { 
                    ...
                    enemy.x += (enemy.x < obs.x + obs.width/2) ? -2 : 2; break; 
                }
            }
        }
        if ((currentRoomId === 8 || currentRoomId === 101) && window.checkCollision(enemy, centerStairs)) { 
        ...
        if (player.heroClass === 'Knight' && player.dashTimer > 0) {
            ...
            if (distToPlayer < player.size + enemy.size + 20) {
                enemy.x += Math.cos(player.faceAngle) * 15;
                enemy.y += Math.sin(player.faceAngle) * 15;
                    ...
                    if (summon.type === 'fusion') {
                            ...
                            currentEnemies.forEach(e => {
                                if (!e.invulnerable && window.checkCollision(hitBox, e)) {
                                    ...
                                    if (!isSwimming) {
                                        ...
                                        if (['elf', 'troll', 'dragon', 'goblin'].includes(e.type.toLowerCase())) bSize /= 2;
                        ...
                        if (minDist < 50) { 
                            ...
                            if (!isSwimming) {
                                ...
                                if (['elf', 'troll', 'dragon', 'goblin'].includes(nearestEnemy.type.toLowerCase())) bSize /= 2;
    ...
    for (let i = currentEnemies.length - 1; i >= 0; i--) {
        if (currentEnemies[i].health <= 0) {
            ...
            if (!isSwimming && e.type !== 'boulder') {
                ...
                if (['elf', 'troll', 'dragon', 'goblin', 'wolf', 'small_golem', 'orc', 'golem', 'gargouille', 'anglerfish', 'siren'].includes(e.type.toLowerCase())) killSize /= 2;
            ...
            if (currentEnemies.length === 0 && currentRoomId !== 999 && currentRoomId !== 301) {
                worldState.clearedRooms[currentRoomId] = true;
                if (typeof hazards !== 'undefined') hazards.length = 0; 
            }
            if (typeof window.updateHUD === 'function') window.updateHUD();
