...
window.level4State = {
    ...
    init: function() {
        ...
        if (typeof player !== 'undefined' && player !== null && player.heroClass === 'Knight') {
            if (!this.cinematicPlayed) {
                ...
                if (typeof window.playCinematic === 'function') {
                    window.playCinematic('knight3.mp4', function() {
                        ...
                        if (typeof gameState !== 'undefined') gameState = "WAITING_MOVE";
                    });
                } else {
                    if (typeof window.loadRoom === 'function') window.loadRoom(302, 'south');
                    if (typeof gameState !== 'undefined') gameState = "WAITING_MOVE";
        ...
        let pSize = (typeof player !== 'undefined' && player !== null && typeof player.size === 'number' && !isNaN(player.size) && player.size > 0) ? player.size : 40;
        ...
        if (!this.cinematicPlayed) {
            this.cinematicPlayed = true;
            let pPrefix = (typeof player !== 'undefined' && player !== null && player.heroClass) ? player.heroClass.toLowerCase() : 'elf';
            if (pPrefix === 'mage') pPrefix = 'burned';
            if (pPrefix === 'necromancer' || pPrefix === 'nécromancien') pPrefix = 'necro';
    ...
    update: function() {
        ...
        let pSize = (typeof player.size === 'number' && !isNaN(player.size) && player.size > 0) ? player.size : 40;

        if (!this.imgLoaded) {
            let floorImg = typeof assetsManager !== 'undefined' && assetsManager.images ? assetsManager.images['OPFLOOR21'] : null;
        ...
        if (!this.hasStarted) {
            ...
            if (typeof keys !== 'undefined' && (keys['z'] || keys['w'] || keys['s'] || keys['q'] || keys['a'] || keys['d'] || keys['arrowup'] || keys['arrowdown'] || keys['arrowleft'] || keys['arrowright'])) {
        ...
        let baseSpeed = (typeof player.speed === 'number' && !isNaN(player.speed)) ? player.speed : 4;
        ...
        for (let trap of this.traps) {
            if (typeof window.checkCollision === 'function' && window.checkCollision(player, trap)) {
                if (trap.type === 'caisse' || trap.type === 'hole_block') {
                    player.x = oldPx; 
                    break;
                }
            }
        ...
        for (let trap of this.traps) {
            if (typeof window.checkCollision === 'function' && window.checkCollision(player, trap)) {
                if (trap.type === 'caisse' || trap.type === 'hole_block' || trap.type === 'hole_death') {
                    player.y = oldPy; 
                    ...
                    if (trap.isActive && trap.hitCooldown <= 0 && (typeof playerInvulnerableTimer === 'undefined' || playerInvulnerableTimer <= 0)) {
                        if (typeof playerStats !== 'undefined') playerStats.health -= 10;
                        trap.hitCooldown = 40;
...
window.updateLevel4 = function() {
    ...
    if (window.level4State && typeof window.level4State.update === 'function') {
        window.level4State.update();
    }
    
    // CORRECTION DU BUG D'INVISIBILITÉ (Timers bloqués)
    if (typeof playerInvulnerableTimer !== 'undefined' && playerInvulnerableTimer > 0) {
        playerInvulnerableTimer--;
    }
    if (typeof window.playerSlowTimer !== 'undefined' && window.playerSlowTimer > 0) {
        window.playerSlowTimer--;
    }
    
    // Optionnel ici, mais par sécurité on laisse le renderer.js s'en occuper
...
window.renderLevel4 = function() {
    ...
    // SÉCURITÉ JOUEUR ABSOLUE
    let pSize = 40;
    if (typeof player !== 'undefined' && player !== null && typeof player.size === 'number' && !isNaN(player.size) && player.size >= 10) {
        pSize = player.size;
    }
    ...
    window.level4State.traps.forEach(trap => {
        ...
        let tImg = typeof window.getAsset === 'function' ? window.getAsset(imgKey) : (typeof assetsManager !== 'undefined' && assetsManager.images ? assetsManager.images[imgKey] : null);
    ...
    // 7. RENDU DU JOUEUR (100% SÉCURISÉ)
    let px = canvas.width/2 - pSize/2;
    let py = canvas.height * 0.4;
    
    if (typeof player !== 'undefined' && player !== null) {
        if (typeof player.x === 'number' && !isNaN(player.x)) px = player.x;
        if (typeof player.y === 'number' && !isNaN(player.y)) py = player.y;
    }
    ...
    let alphaLevel = 1.0;
    if (typeof playerInvulnerableTimer !== 'undefined' && playerInvulnerableTimer > 0) {
        if (Math.floor(playerInvulnerableTimer / 5) % 2 === 0) alphaLevel = 0.3; 
    }
    
    let isElfInvuln = false;
    if (typeof isUltimateActive !== 'undefined' && isUltimateActive && typeof player !== 'undefined' && player !== null && player.heroClass === 'Elf') {
        if (typeof elfStealthBroken === 'undefined' || !elfStealthBroken) isElfInvuln = true;
    }
    
    if (isElfInvuln) alphaLevel = 0.4;
    ...
    let angle = (typeof player !== 'undefined' && player !== null && typeof player.faceAngle === 'number' && !isNaN(player.faceAngle)) ? player.faceAngle : (-Math.PI/2);
    ...
    // Nom de classe propre et sécurisé
    let pClass = (typeof player !== 'undefined' && player !== null && player.heroClass) ? player.heroClass : 'Elf';
    let pPrefix = pClass;
    if (pClass === 'Mage') pPrefix = 'Burned';
    else if (pClass === 'Necromancer' || pClass === 'Nécromancien') pPrefix = 'Necro';
    else pPrefix = pClass.charAt(0).toUpperCase() + pClass.slice(1).toLowerCase();
    ...
    if (img && img.complete && img.naturalWidth > 0) {
        let s = pSize * 3.75;
        if (pClass === 'Knight') s = pSize * 3.0; 
        else if (pClass === 'Elf') s = pSize * 2.25; 
        ctx.drawImage(img, -s/2, -s/2, s, s);
    } else {
        // FALLBACK ULTIME : IMPOSSIBLE À RATER SI L'IMAGE BUG
        ctx.fillStyle = '#39ff14'; // Vert fluo radioactif
        ctx.strokeStyle = '#ffffff';
        ctx.lineWidth = 3;
        ctx.beginPath(); ctx.arc(0, 0, pSize/2 + 5, 0, Math.PI*2); ctx.fill(); ctx.stroke();
        
        ctx.fillStyle = '#ffffff';
        ctx.font = 'bold 14px Arial';
        ctx.textAlign = 'center';
        ctx.fillText("BUG IMG", 0, 5);
    }
