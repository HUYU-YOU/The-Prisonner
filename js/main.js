...
// --- SÉCURITÉS GLOBALES ANTI-CRASH ---
let waveStartDelay = 0;
let arenaShrink = 0;
let currentCrates = []; 

// Désactive le menu Clic Droit pour pouvoir faire le Dash !
document.addEventListener('contextmenu', event => event.preventDefault());

// ============================================================================
// --- GESTION DE LA MUSIQUE (PLAYLIST MP3) ---
// ============================================================================
const playlist = [
...
// Afficher le slider audio au survol
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Connecte n'importe quel bouton qui s'appelle MAP ou PAUSE !
...
document.body.addEventListener('click', () => {
    if (bgMusic.paused && !isMuted) bgMusic.play().catch(e => console.log("Attente audio"));
}, { once: true });
...
// CORRECTION COLLISION : Permet de frôler les murs parfaitement pour les portes
...
// LA FONCTION VITALE QUI FAISAIT CRASHER LES PORTES
...
window.selectHero = function(heroClass) {
    ...
    updatePortrait(heroClass); loadRoom(1); updateHUD(); gameState = "PLAYING";
};
...
function startArenaMode(heroClass) {
    ...
    updatePortrait(heroClass); loadRoom(999); updateHUD(); gameState = "PLAYING";
}
...
window.usePotion = function(color) {
    if (playerStats.inventory.potions[color] > 0) {
        ...
        if (potionUsed) { playerStats.inventory.potions[color]--; updateHUD(); spawnParticles(player.x + player.size/2, player.y + player.size/2, '#3498db', 20); }
    }
};
        ...
        currentEnemies.forEach(enemy => { 
            if (checkCollision(hitBox, enemy)) {
                if (!enemy.invulnerable) {
                    if (enemy.type === 'goblin' && Math.random() < 0.15) { enemy.blockAnimTimer = 45; spawnParticles(enemy.x + enemy.size/2, enemy.y + enemy.size/2, '#bdc3c7', 15); } 
                    else { enemy.health -= 50; spawnParticles(enemy.x + enemy.size/2, enemy.y + enemy.size/2, enemy.color, 15); triggerShake(5, 8); }
                }
            } 
        });
        for (let i = 0; i < currentCrates.length; i++) {
            let obj = currentCrates[i]; if (checkCollision(hitBox, obj) && !obj.isBroken) { obj.health -= 50; }
        }
    }
});
...
function spawnEnemy(type, count, baseX = null, baseY = null) {
    for (let i = 0; i < count; i++) {
        ...
        if (type === 'skeleton') { hp = 50; spd = 2; col = '#bdc3c7'; } else if (type === 'spider') { size = 30; hp = 1; spd = 7; col = '#8e44ad'; } else if (type === 'troll') { size = 80; hp = 900; spd = 2.5; col = '#117a65'; } else if (type === 'mage') { size = 60; hp = 900; spd = 2; col = '#9b59b6'; } else if (type === 'dragon') { size = 150; hp = 3000; spd = 1.0; col = '#8b0000'; }

        if (ex === null || ey === null) {
            let bTop = (currentRoomId === 2 || currentRoomId === 3) ? 250 : wallMargin; let bBot = (currentRoomId === 2 || currentRoomId === 3) ? 550 : canvas.height - wallMargin;
            let minSpawnX = wallMargin + arenaShrink; let maxSpawnX = canvas.width - wallMargin - arenaShrink - size; let minSpawnY = bTop + arenaShrink; let maxSpawnY = bBot - arenaShrink - size;
...
function update() {
        ...
        if (!roomChanged && checkCollision(player, door)) { 
            if (door.locked) {
                if (playerStats.inventory.keys.gold > 0) {
                    playerStats.inventory.keys.gold--; door.locked = false; worldState.unlockedDoors[door.id] = true; updateHUD(); spawnParticles(door.x + door.width/2, door.y + door.height/2, '#f1c40f', 30);
                    if (door.dest !== null) { worldState.openedDoors[door.id] = true; saveRoomState(); loadRoom(door.dest, door.face); player.x = door.spawnX; player.y = door.spawnY; roomChanged = true; }
                } else {
    ...
    if (isUltimateActive) {
        ...
        if (player.heroClass === 'Knight' && ultimateTimer % 60 === 0) { playerStats.health = Math.min(playerStats.maxHealth, playerStats.health + (playerStats.maxHealth * 0.1)); updateHUD(); spawnParticles(player.x + player.size/2, player.y + player.size/2, '#2ecc71', 10); }
    }
    
    if (playerPoisonTimer > 0) { playerPoisonTimer--; if (playerPoisonTimer % 60 === 0 && playerStats.health > 1) { playerStats.health -= 5; if (playerStats.health < 1) playerStats.health = 1; updateHUD(); spawnParticles(player.x + player.size/2, player.y + player.size/2, '#8e44ad', 5); } }
    ...
    // --- CORRECTION DU CHRONO DE VAGUE DANS L'ARÈNE ---
    if (currentRoomId === 999) {
        ...
        if (arenaState === "WAITING" && waveStartDelay <= 0) {
            ...
            if (arenaTimer <= 0) {
                ...
                if (arenaWave === 10) spawnEnemy('troll', 1); else if (arenaWave === 20) spawnEnemy('mage', 1); else if (arenaWave === 30) spawnEnemy('dragon', 1);
                else {
                    if (arenaWave < 5) spawnEnemy('goblin', arenaWave * 2); else if (arenaWave < 15) { spawnEnemy('goblin', 5); spawnEnemy('skeleton', arenaWave - 4); } 
                    else { let numGoblins = 5; let numSkeletons = 10; let numSpiders = 0; let extra = arenaWave - 14; for (let i = 1; i <= extra; i++) { let cycle = i % 3; if (cycle === 1) numSkeletons++; else if (cycle === 2) numSpiders++; else numGoblins++; } spawnEnemy('goblin', numGoblins); spawnEnemy('skeleton', numSkeletons); if (numSpiders > 0) spawnEnemy('spider', numSpiders); }
                }
    ...
    if (player.dashTimer > 0) { player.dashTimer--; dx_mov = player.dashVx; dy_mov = player.dashVy; if (player.dashTimer % 3 === 0) { spawnParticles(player.x + player.size/2, player.y + player.size/2, '#95a5a6', 2); } } 
    else {
        if (keys['q'] || keys['a'] || keys['arrowleft']) dx_mov -= currentSpeedPlayer; if (keys['d'] || keys['arrowright']) dx_mov += currentSpeedPlayer;
        if (keys['z'] || keys['w'] || keys['arrowup']) dy_mov -= currentSpeedPlayer; if (keys['s'] || keys['arrowdown']) dy_mov += currentSpeedPlayer;
    }
    ...
    // CORRECTION : TU PEUX MARCHER SUR LES COFFRES OUVERTS ET CAISSES CASSÉES !
    for (let i = 0; i < currentCrates.length; i++) { if (!currentCrates[i].isBroken && checkCollision(player, currentCrates[i])) { player.x = oldPx; player.dashTimer = 0; break; } } 
    ...
    for (let i = 0; i < currentCrates.length; i++) { if (!currentCrates[i].isBroken && checkCollision(player, currentCrates[i])) { player.y = oldPy; player.dashTimer = 0; break; } }
    ...
    // CORRECTION : SUPPRESSION DÉFINITIVE DES OBJETS (Le point jaune est vaincu !)
    for (let i = currentItems.length - 1; i >= 0; i--) {
        let item = currentItems[i];
        if (checkCollision(player, item)) {
            worldState.collectedItems[item.id] = true; 
            if (item.type === 'key') playerStats.inventory.keys.gold++; else if (item.type === 'potion_green') playerStats.inventory.potions.green++; else if (item.type === 'key_skull') playerStats.inventory.keys.skull++; 
            else if (item.type === 'coin') { 
                playerStats.inventory.coins++; 
                localStorage.setItem('kebra_coins', playerStats.inventory.coins); 
                let coinUi = document.getElementById('inv-coins');
                if (coinUi) coinUi.innerText = playerStats.inventory.coins;
            }
    ...
    if (attackCooldown > 0) attackCooldown--; if (player.heroClass === 'Knight' && attackCooldown < 25) isAttacking = false;
    let isElfInvuln = (isUltimateActive && player.heroClass === 'Elf' && !elfStealthBroken);
    
    // --- GESTION DES CAISSES ET COFFRES (LOOT DEVANT !) ---
    for (let i = 0; i < currentCrates.length; i++) {
        ...
        if (!crate.isBroken && crate.health <= 0) {
            ...
            if (crate.type === 'chest') {
                ...
                // Le loot apparaît juste EN BAS du coffre (pour ne pas être dedans)
                ...
                currentItems.push({ id: 'coin_chest1_' + Date.now(), type: 'coin', x: crate.x + 40, y: crate.y + crate.size + 5, size: 8, collected: false });
                currentItems.push({ id: 'coin_chest2_' + Date.now(), type: 'coin', x: crate.x - 10, y: crate.y + crate.size + 5, size: 8, collected: false });
    ...
    currentEnemies.forEach((enemy) => {
        if (enemy.attackAnimTimer === undefined) enemy.attackAnimTimer = 0; if (enemy.blockAnimTimer === undefined) enemy.blockAnimTimer = 0; if (enemy.ultiAnimTimer === undefined) enemy.ultiAnimTimer = 0; if (enemy.dashTimer === undefined) { enemy.dashTimer = 180; enemy.isDashing = 0; }
        if (enemy.attackAnimTimer > 0) enemy.attackAnimTimer--; if (enemy.blockAnimTimer > 0) enemy.blockAnimTimer--; if (enemy.ultiAnimTimer > 0) enemy.ultiAnimTimer--; enemy.wobble += 0.1; 

        let targetX = player.x; let targetY = player.y; let targetEntity = player; let minDistToTarget = 9999;
        if (targetEntity === player && !isElfInvuln) minDistToTarget = Math.hypot(targetX - enemy.x, targetY - enemy.y); else if (targetEntity === player && isElfInvuln) minDistToTarget = 9999; 
        let dx = 0, dy = 0, dist = minDistToTarget; if (dist !== 9999) { dx = targetX - enemy.x; dy = targetY - enemy.y; }

        let currentEnemySpeed = enemy.speed; if (enemy.slowTimer > 0 || enemy.isPermanentlySlowed) currentEnemySpeed *= 0.5; 
        let dx_mov = 0, dy_mov = 0; if (dist > 0 && dist < 9999) { dx_mov = (dx / dist) * currentEnemySpeed; dy_mov = (dy / dist) * currentEnemySpeed; }
        ...
        if (enemy.x < minLimitX) enemy.x = minLimitX; if (enemy.y < minLimitY) enemy.y = minLimitY; if (enemy.x > eMaxX) enemy.x = eMaxX; if (enemy.y > eMaxY) enemy.y = eMaxY;

        if (targetEntity === player && playerInvulnerableTimer <= 0 && !enemy.invulnerable && checkCollision(player, enemy)) {
            playerStats.health -= 20; triggerShake(12, 20); spawnParticles(player.x + player.size/2, player.y + player.size/2, '#e74c3c', 25);
            playerInvulnerableTimer = 60; updateHUD(); if (playerStats.health <= 0) handlePlayerDeath();
        }
    });
    ...
    for (let i = currentEnemies.length - 1; i >= 0; i--) {
        if (currentEnemies[i].health <= 0) {
            ...
            if (e.type === 'troll' && currentRoomId === 8 && !worldState.bossDefeated) { worldState.bossDefeated = true; currentItems.push({ id: 'boss_key', type: 'key_skull', x: canvas.width/2 - 10, y: canvas.height/2 + 80, size: 20, collected: false }); triggerShake(20, 30); }
            if (Math.random() < 0.3 && !['troll', 'mage', 'dragon'].includes(e.type)) { currentItems.push({ id: 'coin_en_' + Date.now() + i, type: 'coin', x: e.x + e.size/2, y: e.y + e.size/2, size: 8, collected: false }); }
            playerStats.mana = Math.min(100, playerStats.mana + 5); spawnParticles(e.x + e.size/2, e.y + e.size/2, '#c0392b', 30); currentEnemies.splice(i, 1);
            if (currentEnemies.length === 0 && currentRoomId !== 999) worldState.clearedRooms[currentRoomId] = true; updateHUD();
        }
    }
    ...
    for (let i = projectiles.length - 1; i >= 0; i--) {
        ...
        for (let j = 0; j < currentEnemies.length; j++) {
            ...
            if (!projectileHit && checkCollision(arrowHitbox, enemy)) {
                if (!enemy.invulnerable) {
                    let isBlocked = false;
                    if (enemy.type === 'goblin' && Math.random() < 0.15) { isBlocked = true; enemy.blockAnimTimer = 45; spawnParticles(enemy.x + enemy.size/2, enemy.y + enemy.size/2, '#bdc3c7', 15); }
                ...
                if (isPiercingElf || player.heroClass === 'Mage') { if (!p.hitTargets) p.hitTargets = []; p.hitTargets.push(enemy); } else { projectileHit = true; break; } 
            }
        }
    ...
    if (currentRoomId === 8 && worldState && worldState.bossDefeated) {
        ...
        if (checkCollision(player, triggerStairs)) {
            if (playerStats.inventory.keys.skull > 0) {
                playerStats.inventory.keys.skull--; setTimeout(() => { alert("FÉLICITATIONS !\n\nLa suite de l'aventure arrive très bientôt..."); window.location.reload(); }, 100); return;
            }
        }
    }

    renderGameView(); requestAnimationFrame(update);
}

// CORRECTION: RALLONGEMENT DES PORTES (+15px) POUR NE PLUS ÊTRE BLOQUÉ !
function loadRoom(roomId, entryFace = 'south') {
  ...
  if (!worldState.bloodStains) worldState.bloodStains = {}; if (!worldState.visitedRooms) worldState.visitedRooms = {};
  if (!worldState.brokenCrates) worldState.brokenCrates = {}; if (!worldState.openedChests) worldState.openedChests = {};
  if (!worldState.bloodStains[roomId]) worldState.bloodStains[roomId] = []; bloodStains = worldState.bloodStains[roomId];

  worldState.visitedRooms[roomId] = true;
  let bTop = (roomId === 2 || roomId === 3) ? 250 : wallMargin; let bBot = (roomId === 2 || roomId === 3) ? 550 : 800 - wallMargin;
