window.update = function() {
    ...
    if (gameState === "PAUSED" || (gameState !== "PLAYING" && gameState !== "GAMEOVER")) { requestAnimationFrame(window.update); return; }
    ...
    if (doorToPass) {
        ...
        if (typeof currentDoors !== 'undefined') {
            currentDoors.forEach(d => { if (d.face === returnFace) { worldState.openedDoors[d.id] = true; d.locked = false; } });
        }

        player.x = doorToPass.spawnX; player.y = doorToPass.spawnY;
        requestAnimationFrame(window.update); return;
    }
    
    if ((keys['space'] || keys['0'] || keys['control']) && playerStats.mana >= 100) {
        if (typeof window.activateUltimate === 'function') window.activateUltimate(); keys['space'] = false; keys['0'] = false; keys['control'] = false; 
    }
    
    if (typeof leftClickHeld !== 'undefined' && leftClickHeld) {
        if (typeof leftClickHoldTime === 'undefined') leftClickHoldTime = 0; leftClickHoldTime++;
        if (leftClickHoldTime >= 180 && playerStats.mana >= 100) { if (typeof window.activateUltimate === 'function') window.activateUltimate(); leftClickHeld = false; }
    }
    ...
    if (typeof isUltimateActive !== 'undefined' && isUltimateActive) {
        if (typeof ultimateTimer !== 'undefined') {
            ...
            if (player.heroClass === 'Knight' && ultimateTimer % 60 === 0) { playerStats.health = Math.min(playerStats.maxHealth, playerStats.health + (playerStats.maxHealth * 0.1)); if (typeof window.updateHUD === 'function') window.updateHUD(); }
        }
    }
    
    if (typeof playerPoisonTimer !== 'undefined' && playerPoisonTimer > 0) {
        playerPoisonTimer--; if (playerPoisonTimer % 60 === 0 && playerStats.health > 1) { playerStats.health -= 5; if (playerStats.health < 1) playerStats.health = 1; if (typeof window.updateHUD === 'function') window.updateHUD(); }
    }
    ...
    if (currentRoomId === 8 && typeof window.checkCollision === 'function' && window.checkCollision(player, centerStairs)) { 
        if (!worldState.bossDefeated || (!worldState.level2Unlocked && playerStats.inventory.keys.skull <= 0)) { player.x = oldPx; player.dashTimer = 0; }
    } 
    ...
    if (typeof currentObstacles !== 'undefined') {
        for (let i = 0; i < currentObstacles.length; i++) {
            ...
            if (typeof window.checkCollision === 'function' && window.checkCollision(player, obs)) {
                if (obs.type === 'water') { alert("DIRECTION NIVEAU 3 ! (Prochainement...)"); player.y += 20; break; } 
                else if (obs.type === 'hole') { if (player.dashTimer <= 0) { player.x = oldPx; player.dashTimer = 0; break; } } 
    ...
    if (currentRoomId === 8 && typeof window.checkCollision === 'function' && window.checkCollision(player, centerStairs)) { 
        if (!worldState.bossDefeated || (!worldState.level2Unlocked && playerStats.inventory.keys.skull <= 0)) { player.y = oldPy; player.dashTimer = 0; }
    } 
    ...
    if (typeof currentObstacles !== 'undefined') {
        for (let i = 0; i < currentObstacles.length; i++) {
            let obs = currentObstacles[i];
            if (typeof window.checkCollision === 'function' && window.checkCollision(player, obs)) {
                if (obs.type === 'hole') { if (player.dashTimer <= 0) { player.y = oldPy; player.dashTimer = 0; break; } } 
                else if (obs.type !== 'water') { player.y = oldPy; player.dashTimer = 0; break; }
            }
        }
    }
    ...
    let minLimitX = bLeft + arenaShrink; let minLimitY = bTop + arenaShrink;
    let maxLimitX = bRight - arenaShrink - player.size; let maxLimitY = bBot - arenaShrink - player.size;
    ...
    if (typeof particles !== 'undefined') { for (let i = particles.length - 1; i >= 0; i--) { let p = particles[i]; p.x += p.vx; p.y += p.vy; p.life -= 0.03; if (p.life <= 0) particles.splice(i, 1); } }
    if (typeof bloodStains !== 'undefined') { for (let i = bloodStains.length - 1; i >= 0; i--) { let b = bloodStains[i]; if (b.life === undefined) b.life = (currentRoomId === 999) ? 1200 : 3600; b.life--; if (b.life < 300) { b.opacity = b.life / 300; } else { b.opacity = 1.0; } if (b.life <= 0) bloodStains.splice(i, 1); } }
    ...
    // INTERACTION ESCALIER SALLE 8 -> NIVEAU 2
    if (currentRoomId === 8 && worldState && worldState.bossDefeated) {
        ...
        if (typeof window.checkCollision === 'function' && window.checkCollision(player, triggerStairs)) {
            if (worldState.level2Unlocked || playerStats.inventory.keys.skull > 0) {
                if (!worldState.level2Unlocked) { playerStats.inventory.keys.skull--; worldState.level2Unlocked = true; }
                if (typeof window.saveRoomState === 'function') window.saveRoomState();
                if (typeof window.loadRoom === 'function') window.loadRoom(100, 'south');
                player.x = canvas.width / 2 - player.size / 2; player.y = canvas.height / 2 + 80; player.dashTimer = 0; 
                if (typeof window.updateHUD === 'function') window.updateHUD(); 
    ...
    // INTERACTION ESCALIER SALLE 100 -> RETOUR AU NIVEAU 1
