...
function renderGameView() {
    ...
    if (currentRoomId === 8) {
        ...
        if (sImg && sImg.complete && sImg.naturalWidth > 0) {
            ...
            if (!worldState.bossDefeated) { 
                ...
                ctx.fillStyle = '#e74c3c'; ctx.fillText("BLOQUÉ !", sx + sw/2, sy + sh/2 + 20); 
            } else { 
                ctx.fillStyle = '#f1c40f'; ctx.fillText("SORTIE ICI", sx + sw/2, sy + sh/2 + 5); 
            }
            ctx.textAlign = 'left';
        }
        if (worldState.bossDefeated) { 
    ...
    if (typeof necroSummons !== 'undefined') {
        necroSummons.forEach(s => {
            ...
            if (s.type === 'fusion') { 
                ...
                ctx.fillStyle = '#8e44ad'; ctx.beginPath(); ctx.arc(-s.size/4, -s.size/4, 4, 0, Math.PI*2); ctx.fill(); 
                ctx.beginPath(); ctx.arc(s.size/4, -s.size/4, 4, 0, Math.PI*2); ctx.fill(); 
            }
    ...
    currentEnemies.forEach(enemy => {
        ...
        if (enemy.ultiAnimTimer > 0) { 
            ...
            if (imgUlt && imgUlt.complete && imgUlt.naturalWidth > 0) { 
                ...
                ctx.globalAlpha = enemy.ultiAnimTimer / 30; ctx.rotate(progress * Math.PI); 
    ...
    if (boss && currentRoomId === 8 && !worldState.bossDefeated) {
        ...
        let barWidth = 600; let barHeight = 24; let bx = canvas.width/2 - barWidth/2; let by = 30; 
        
        ctx.fillStyle = '#111'; ctx.fillRect(bx, by, barWidth, barHeight); 
    ...
    projectiles.forEach(p => { 
        ...
        if (p.isNecro) { 
            ...
            if (imgFire && imgFire.complete && imgFire.naturalWidth > 0) { 
                ctx.globalCompositeOperation = 'screen'; ctx.rotate(Math.PI / 2); 
                let pSize = p.size * 5; ctx.drawImage(imgFire, -pSize/2, -pSize/2, pSize, pSize); 
            } else { 
