...
// MISE A JOUR DU HUD DE L'INVENTAIRE HTML
function updateHUD() {
...
function renderGameView() {
    ...
    if (shakeTimer > 0) { let dx = (Math.random() - 0.5) * shakeIntensity * 2; let dy = (Math.random() - 0.5) * shakeIntensity * 2; ctx.translate(dx, dy); shakeTimer--; shakeIntensity *= 0.9; }
    
    let imageSol = assetsManager.images['sol_base']; ctx.fillStyle = '#2c251f'; ctx.fillRect(0, 0, canvas.width, canvas.height);
    if (imageSol && imageSol.complete && imageSol.naturalWidth > 0) { ctx.fillStyle = ctx.createPattern(imageSol, 'repeat'); ctx.fillRect(0, 0, canvas.width, canvas.height); } else { ctx.strokeStyle = '#3d342c'; ctx.lineWidth = 1; for(let i = 0; i < canvas.width; i += 60) { for(let j = 0; j < canvas.height; j += 60) { ctx.strokeRect(i, j, 60, 60); } } }
    ...
    if (currentRoomId === 2 || currentRoomId === 3) {
        ctx.fillStyle = '#0a0a0a'; ctx.fillRect(0, 0, canvas.width, bTop - wallMargin); ctx.fillRect(0, bBot + wallMargin, canvas.width, canvas.height - bBot - wallMargin); 
        ctx.strokeStyle = '#3d342c'; ctx.lineWidth = 6; ctx.beginPath(); ctx.moveTo(0, bTop - wallMargin); ctx.lineTo(canvas.width, bTop - wallMargin); ctx.stroke(); ctx.beginPath(); ctx.moveTo(0, bBot + wallMargin); ctx.lineTo(canvas.width, bBot + wallMargin); ctx.stroke();
    }
    ...
    if (currentRoomId === 8) {
        ...
        ctx.save();
        if (sImg && sImg.complete && sImg.naturalWidth > 0) { ctx.drawImage(sImg, sx, sy, sw, sh); if (!worldState.bossDefeated) { ctx.fillStyle = 'rgba(0, 0, 0, 0.6)'; ctx.fillRect(sx, sy, sw, sh); } } else { ctx.fillStyle = '#111'; ctx.fillRect(sx, sy, sw, sh); ctx.strokeStyle = '#555'; ctx.lineWidth = 6; ctx.strokeRect(sx, sy, sw, sh); ctx.fillStyle = '#fff'; ctx.font = 'bold 20px Arial'; ctx.textAlign = 'center'; if (!worldState.bossDefeated) { ctx.fillText("ESCALIER", sx + sw/2, sy + sh/2 - 10); ctx.fillStyle = '#e74c3c'; ctx.fillText("BLOQUÉ !", sx + sw/2, sy + sh/2 + 20); } else { ctx.fillStyle = '#f1c40f'; ctx.fillText("SORTIE ICI", sx + sw/2, sy + sh/2 + 5); } ctx.textAlign = 'left'; }
    ...
    if (currentRoomId === 1) {
        ...
        if (typeof bookshelf !== 'undefined') {
            ...
            if (distToBiblio < 120) {
                ctx.save(); ctx.fillStyle = 'rgba(0, 0, 0, 0.8)'; ctx.strokeStyle = '#f1c40f'; ctx.lineWidth = 2; let popW = 320; let popH = 40; let popX = bookshelf.x + bookshelf.width/2 - popW/2; let popY = bookshelf.y - popH - 15; popY += Math.sin(Date.now() / 200) * 3; ctx.fillRect(popX, popY, popW, popH); ctx.strokeRect(popX, popY, popW, popH); ctx.fillStyle = '#ecf0f1'; ctx.font = 'bold 14px Arial'; ctx.textAlign = 'center'; ctx.fillText("Appuyez sur 'E' pour lire un vieux grimoire...", popX + popW/2, popY + 25); ctx.textAlign = 'left'; ctx.restore();
            }
        }
    }
    ...
    if (typeof currentCrates !== 'undefined') {
        currentCrates.forEach(crate => {
            let imgName = '';
            if (crate.type === 'barrel') imgName = crate.isBroken ? 'crate2' : 'crate1'; else if (crate.type === 'box') imgName = crate.isBroken ? 'crate4' : 'crate3'; else if (crate.type === 'chest') imgName = crate.isBroken ? 'chest2' : 'chest1';
            ...
            if (img && img.complete && img.naturalWidth > 0) { ctx.drawImage(img, -crate.size/2, -crate.size/2, crate.size, crate.size); } else { ctx.fillStyle = crate.isBroken ? '#5c4033' : '#8B4513'; if (crate.type === 'chest') ctx.fillStyle = crate.isBroken ? '#7f8c8d' : '#f1c40f'; ctx.fillRect(-crate.size/2, -crate.size/2, crate.size, crate.size); if (!crate.isBroken) { ctx.strokeStyle = '#000'; ctx.lineWidth = 2; ctx.strokeRect(-crate.size/2, -crate.size/2, crate.size, crate.size); } }
            ctx.restore();
        });
    }
    ...
    currentDoors.forEach(door => {
        ...
        if (doorImg && doorImg.complete && doorImg.naturalWidth > 0) { ctx.drawImage(doorImg, door.x, door.y, door.width, door.height); if (currentRoomId === 8 && !worldState.bossDefeated && door.face === 'south') { ctx.fillStyle = 'rgba(192, 57, 43, 0.4)'; ctx.fillRect(door.x, door.y, door.width, door.height); } } else { ctx.fillStyle = isOpen ? '#1a110c' : '#3e2a1d'; ctx.fillRect(door.x, door.y, door.width, door.height); ctx.strokeStyle = '#111'; ctx.lineWidth = 2; if(door.face === 'north' || door.face === 'south') { ctx.strokeRect(door.x + 10, door.y, door.width - 20, door.height); } else { ctx.strokeRect(door.x, door.y + 10, door.width, door.height - 20); } }
    });
    ...
    if (typeof necroSummons !== 'undefined') {
        necroSummons.forEach(s => {
            ...
            if (s.type === 'fusion') { if (s.invulnerableTimer && s.invulnerableTimer > 0) { ctx.shadowBlur = 20; ctx.shadowColor = '#f1c40f'; ctx.strokeStyle = '#f1c40f'; ctx.lineWidth = 4; ctx.beginPath(); ctx.arc(0, 0, s.size/2 + 8, 0, Math.PI*2); ctx.stroke(); } ctx.fillStyle = '#8e44ad'; ctx.fillRect(-s.size/2, -s.size/2, s.size, s.size); ctx.fillStyle = '#1abc9c'; ctx.beginPath(); ctx.arc(0, 0, s.size/4, 0, Math.PI*2); ctx.fill(); } else { ctx.fillStyle = 'rgba(44, 62, 80, 0.7)'; ctx.beginPath(); ctx.arc(0, 0, s.size/2, 0, Math.PI*2); ctx.fill(); ctx.fillStyle = '#8e44ad'; ctx.beginPath(); ctx.arc(-s.size/4, -s.size/4, 4, 0, Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(s.size/4, -s.size/4, 4, 0, Math.PI*2); ctx.fill(); }
    ...
    projectiles.forEach(p => { 
        ...
        if (p.isNecro) { ctx.fillStyle = '#8e44ad'; ctx.beginPath(); ctx.arc(0, 0, p.size, 0, Math.PI*2); ctx.fill(); ctx.fillStyle = '#2c3e50'; ctx.beginPath(); ctx.arc(-2, 0, p.size-2, 0, Math.PI*2); ctx.fill(); } else if (p.isFire) { let imgFire = assetsManager.images['Attack_fire_mage']; if (imgFire && imgFire.complete && imgFire.naturalWidth > 0) { ctx.globalCompositeOperation = 'screen'; ctx.rotate(Math.PI / 2); let pSize = p.size * 5; ctx.drawImage(imgFire, -pSize/2, -pSize/2, pSize, pSize); } else { ctx.fillStyle = '#e67e22'; ctx.beginPath(); ctx.arc(0, 0, p.size, 0, Math.PI*2); ctx.fill(); ctx.fillStyle = '#f1c40f'; ctx.beginPath(); ctx.arc(-2, 0, p.size-2, 0, Math.PI*2); ctx.fill(); } } else { ctx.fillStyle = '#bdc3c7'; ctx.fillRect(-8, -1, 16, 2); ctx.fillStyle = '#ecf0f1'; ctx.beginPath(); ctx.moveTo(8, -3); ctx.lineTo(14, 0); ctx.lineTo(8, 3); ctx.fill(); ctx.fillStyle = '#34495e'; ctx.beginPath(); ctx.moveTo(-8, -3); ctx.lineTo(-12, -3); ctx.lineTo(-8, 0); ctx.lineTo(-12, 3); ctx.lineTo(-8, 3); ctx.fill(); }
        ctx.restore();
    });
    
    // --- L'OS CLASSIQUE ET PROPRE (SANS BRILLANCE) ---
    enemyProjectiles.forEach(p => { 
        ctx.save(); ctx.translate(p.x, p.y); let pAngle = Math.atan2(p.vy, p.vx); 
        if (p.type === 'bone') { 
            ctx.rotate(pAngle); ctx.shadowBlur = 0; ctx.fillStyle = '#ecf0f1'; 
            let l = p.size * 1.5; let w = p.size * 0.3; let r = p.size * 0.6; 
            ctx.fillRect(-l, -w, l * 2, w * 2); 
            ctx.beginPath(); ctx.arc(-l, -w*1.2, r, 0, Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(-l, w*1.2, r, 0, Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(l, -w*1.2, r, 0, Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(l, w*1.2, r, 0, Math.PI*2); ctx.fill(); 
            ctx.fillStyle = '#bdc3c7'; ctx.fillRect(-l + 2, -w/4, l*2 - 4, w/2); 
            ...
            ctx.shadowColor = p.color; ctx.shadowBlur = 10; ctx.rotate(pAngle); ctx.fillStyle = p.color; ctx.beginPath(); ctx.arc(0, 0, p.size, 0, Math.PI * 2); ctx.fill(); 
        }
        ctx.restore();
    });
    ...
    if (drawPlayer) {
        ...
        if (player.heroClass === 'Elf') {
            ctx.rotate(tilt); let angle = player.faceAngle; let skin = 'Elf_front'; if (angle > -Math.PI/4 && angle <= Math.PI/4) skin = 'Elf_est'; else if (angle > Math.PI/4 && angle <= 3*Math.PI/4) skin = 'Elf_front'; else if (angle > -3*Math.PI/4 && angle <= -Math.PI/4) skin = 'Elf_back'; else skin = 'Elf_west';                                                     
            let img = assetsManager.images[skin]; let displaySize = player.size * 6.0; if (img && img.complete && img.naturalWidth > 0) { ctx.drawImage(img, -displaySize/2, -displaySize/2, displaySize, displaySize); } else { ctx.rotate(player.faceAngle); ctx.fillStyle = '#2ecc71'; ctx.beginPath(); ctx.arc(0, 0, player.size/2, 0, Math.PI*2); ctx.fill(); }
        } else if (player.heroClass === 'Mage') {
            ctx.rotate(player.faceAngle + tilt - (Math.PI / 2)); let imgMage = assetsManager.images['Burned_top_view']; let displaySize = player.size * 3.5; if (imgMage && imgMage.complete && imgMage.naturalWidth > 0) { ctx.save(); ctx.beginPath(); ctx.arc(0, 0, displaySize/2.2, 0, Math.PI*2); ctx.clip(); ctx.drawImage(imgMage, -displaySize/2, -displaySize/2, displaySize, displaySize); ctx.restore(); } else { ctx.rotate(Math.PI / 2); ctx.fillStyle = playerPoisonTimer > 0 ? '#27ae60' : '#e67e22'; ctx.beginPath(); ctx.arc(0, 0, player.size/2, 0, Math.PI*2); ctx.fill(); }
        } else {
            ...
            if (player.heroClass === 'Necromancer') { ctx.fillStyle = '#2c3e50'; ctx.beginPath(); ctx.moveTo(-10, -10); ctx.lineTo(15, 0); ctx.lineTo(-10, 10); ctx.fill(); ctx.strokeStyle = '#8e44ad'; ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(5, 0, 15, -Math.PI/2, Math.PI/2); ctx.stroke(); } else if (player.heroClass === 'Knight') { ctx.fillStyle = '#bdc3c7'; ctx.fillRect(-15, -15, 30, 30); ctx.fillStyle = '#2c3e50'; ctx.fillRect(0, -10, 5, 20); ctx.save(); if(isAttacking) { ctx.translate(15, 15); ctx.rotate(Math.PI/4); } else { ctx.translate(5, 20); } ctx.fillStyle = '#f1c40f'; ctx.fillRect(0, -4, 10, 8); ctx.fillStyle = '#ecf0f1'; ctx.fillRect(10, -2, 35, 4); ctx.restore(); if (isAttacking) { ctx.strokeStyle = 'rgba(236, 240, 241, 0.8)'; ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(0, 0, player.size + 15, -Math.PI/3, Math.PI/3); ctx.stroke(); } }
        }
        ctx.restore(); ctx.globalAlpha = 1.0; 
    }

    // --- CORRECTION PARTICULES QUI FIGENT ET LAISSENT DES POINTS JAUNES ---
    for (let i = particles.length - 1; i >= 0; i--) {
        let p = particles[i]; 
        p.x += p.vx; p.y += p.vy; 
        p.life -= 0.05; // Cette ligne empêche la particule de rester figée !
        if (p.life <= 0) {
    ...
    gradient.addColorStop(0, 'rgba(0, 0, 0, 0)'); gradient.addColorStop(0.4, 'rgba(0, 0, 0, 0.4)'); gradient.addColorStop(1, 'rgba(0, 0, 0, 0.9)');     
    ctx.fillStyle = gradient; ctx.fillRect(0, 0, canvas.width, canvas.height); 
    ctx.restore();

    if (currentRoomId === 999) {
        ctx.fillStyle = '#ecf0f1'; ctx.font = 'bold 28px Arial'; ctx.textAlign = 'center'; let displayWave = arenaState === "WAITING" ? arenaWave : arenaWave - 1;
        if (arenaState === "WAITING" && arenaTimer > 0) ctx.fillText("VAGUE " + displayWave + " DANS " + Math.ceil(arenaTimer/60) + "S", canvas.width/2, wallMargin + 40); else if (displayWave > 0) ctx.fillText("VAGUE " + displayWave, canvas.width/2, wallMargin + 40); ctx.textAlign = 'left';
    }
    ctx.restore(); 
}
