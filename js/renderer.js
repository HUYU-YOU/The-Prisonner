...
// OUTIL ANTI-BUG MAJUSCULE/MINUSCULE
...
window.renderGameView = function() {
    ...
    if (imageSol && imageSol.complete && imageSol.naturalWidth > 0) { 
        ...
        for(let i = 0; i < canvas.width; i += 60) { for(let j = 0; j < canvas.height; j += 60) { ctx.strokeRect(i, j, 60, 60); } } 
    }
    ...
    if (currentRoomId === 8 || currentRoomId === 999) { 
    ...
    if (currentRoomId === 1) {
        ...
        if (typeof bookshelf !== 'undefined') {
            ...
            if (distToBiblio < 120) {
                ...
                let popW = 320; let popH = 40; let popX = bookshelf.x + bookshelf.width/2 - popW/2; let popY = bookshelf.y - popH - 15; popY += Math.sin(Date.now() / 200) * 3; 
                ctx.fillRect(popX, popY, popW, popH); ctx.strokeRect(popX, popY, popW, popH); 
                ctx.fillStyle = '#ecf0f1'; ctx.font = 'bold 14px Arial'; ctx.textAlign = 'center'; ctx.fillText("Appuyez sur 'E' pour lire un vieux grimoire...", popX + popW/2, popY + 25); ctx.textAlign = 'left'; ctx.restore();
            }
        }
    }
    ...
    if (typeof currentCrates !== 'undefined') {
        currentCrates.forEach(crate => {
            ...
            else { ctx.fillStyle = crate.isBroken ? '#5c4033' : '#8B4513'; if (crate.type === 'chest') ctx.fillStyle = crate.isBroken ? '#7f8c8d' : '#f1c40f'; ctx.fillRect(-crate.size/2, -crate.size/2, crate.size, crate.size); if (!crate.isBroken) { ctx.strokeStyle = '#000'; ctx.lineWidth = 2; ctx.strokeRect(-crate.size/2, -crate.size/2, crate.size, crate.size); } }
            ctx.restore();
        });
    }
    ...
    currentItems.forEach(item => {
        if (!item.collected) {
            ...
            if (item.type === 'key') { ctx.fillStyle = '#f1c40f'; ctx.beginPath(); ctx.arc(item.x, item.y + floatY, 8, 0, Math.PI * 2); ctx.fill(); ctx.fillRect(item.x + 6, item.y - 3 + floatY, 18, 6); ctx.fillRect(item.x + 18, item.y + 3 + floatY, 3, 6); } 
                ...
                if (coinImg && coinImg.complete && coinImg.naturalWidth > 0) { ctx.drawImage(coinImg, -20, -20, 40, 40); } 
                else { ctx.fillStyle = '#f1c40f'; ctx.beginPath(); ctx.arc(0, 0, item.size, 0, Math.PI*2); ctx.fill(); }
            }
            ctx.restore();
        }
    });
    ...
    if (typeof necroSummons !== 'undefined') {
        necroSummons.forEach(s => {
            ...
            if (s.type === 'fusion') { ctx.fillStyle = '#8e44ad'; ctx.fillRect(-s.size/2, -s.size/2, s.size, s.size); } 
    ...
    currentEnemies.forEach(enemy => {
        ...
        if (!img || !img.complete || img.naturalWidth === 0) { 
            ...
            if (lowPrefix === 'goblin') { if (enemy.blockAnimTimer > 0) fallbackName = 'goblin_top_block'; else fallbackName = 'goblin_top_view'; } 
        ...
        if (img && img.complete && img.naturalWidth > 0) { ctx.drawImage(img, -enemy.size, -enemy.size, enemy.size * 2, enemy.size * 2); } 
        else { ctx.fillStyle = '#e74c3c'; ctx.fillRect(-enemy.size/2, -enemy.size/2, enemy.size, enemy.size); }
        
        if (enemy.ultiAnimTimer > 0) { 
            ctx.save(); ctx.globalCompositeOperation = 'screen'; let imgUlt = window.getAsset('Ulti_fire_mage'); 
            if (imgUlt && imgUlt.complete && imgUlt.naturalWidth > 0) { let expSize = enemy.size * 3.5; ctx.drawImage(imgUlt, -expSize/2, -expSize/2, expSize, expSize); } 
            ctx.restore(); 
        }
        ctx.restore(); 
    ...
    enemyProjectiles.forEach(p => { 
        ...
        if (p.type === 'bone') { ctx.rotate(pAngle); ctx.fillStyle = '#ecf0f1'; let l = p.size * 1.5; let w = p.size * 0.3; let r = p.size * 0.6; ctx.fillRect(-l, -w, l * 2, w * 2); ctx.beginPath(); ctx.arc(-l, -w*1.2, r, 0, Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(-l, w*1.2, r, 0, Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(l, -w*1.2, r, 0, Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(l, w*1.2, r, 0, Math.PI*2); ctx.fill(); ctx.fillStyle = '#bdc3c7'; ctx.fillRect(-l + 2, -w/4, l*2 - 4, w/2); } 
        else if (p.type === 'bat_web') { ctx.shadowColor = p.color; ctx.shadowBlur = 10; ctx.rotate(pAngle); ctx.fillStyle = 'rgba(142, 68, 173, 0.8)'; ctx.beginPath(); ctx.moveTo(8, 0); ctx.lineTo(0, -8); ctx.lineTo(-4, -4); ctx.lineTo(-8, -8); ctx.lineTo(-4, 0); ctx.lineTo(-8, 8); ctx.lineTo(-4, 4); ctx.lineTo(0, 8); ctx.closePath(); ctx.fill(); ctx.strokeStyle = '#ecf0f1'; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(0,0); ctx.lineTo(0, -8); ctx.stroke(); ctx.beginPath(); ctx.moveTo(0,0); ctx.lineTo(-8, -8); ctx.stroke(); ctx.beginPath(); ctx.moveTo(0,0); ctx.lineTo(-8, 8); ctx.stroke(); ctx.beginPath(); ctx.moveTo(0,0); ctx.lineTo(0, 8); ctx.stroke(); } 
        else if (p.type === 'fire') { ctx.shadowColor = p.color; ctx.shadowBlur = 10; let fireImg = window.getAsset('Attack_fire_mage'); if (fireImg && fireImg.complete && fireImg.naturalWidth > 0) { ctx.rotate(pAngle + Math.PI / 2); ctx.globalCompositeOperation = 'screen'; let dSize = p.size * 5.0; ctx.drawImage(fireImg, -dSize/2, -dSize/2, dSize, dSize); } else { ctx.rotate(pAngle); ctx.fillStyle = p.color; ctx.beginPath(); ctx.arc(0, 0, p.size, 0, Math.PI * 2); ctx.fill(); } } 
    ...
    if (drawPlayer) {
        ...
        if (attackCooldown > 0) {
            ...
            actionP = attackCooldown > midTime ? 'attack1' : 'attack2';
        }
        ...
        if (!pImg || !pImg.complete || pImg.naturalWidth === 0) {
            pImg = window.getAsset(`${prefixP}_${dirP}_view`);
        }
    ...
    if (playerStats.inventory.coins !== undefined) {
        ...
        ctx.fillStyle = '#fff'; ctx.font = 'bold 24px Arial'; ctx.fillText("x " + playerStats.inventory.coins, wallMargin + 55, 43);
    }
    ...
    if (currentRoomId === 999) {
        ...
        if (displayWave > 0) ctx.fillText("VAGUE " + displayWave, canvas.width/2, wallMargin + 40);
        ctx.textAlign = 'left';
    }
    ctx.restore(); 
};
