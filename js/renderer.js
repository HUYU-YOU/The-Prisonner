...
window.renderGameView = function() {
    ...
    bloodStains.forEach(blood => { 
        ctx.fillStyle = 'rgba(100, 0, 0, 0.7)'; ctx.beginPath(); ctx.arc(blood.x + 10, blood.y + 10, blood.r || 15, 0, Math.PI * 2); ctx.fill(); 
    });
    ...
    if (currentRoomId === 8) {
        ...
        if (sImg && sImg.complete && sImg.naturalWidth > 0) { 
            ...
            else { ctx.fillStyle = '#f1c40f'; ctx.fillText("SORTIE ICI", sx + sw/2, sy + sh/2 + 5); } ctx.textAlign = 'left'; 
        }
    ...
    if (currentRoomId === 1) {
        ...
        if (typeof bookshelf !== 'undefined') {
            ...
            if (distToBiblio < 120) {
                ctx.save(); ctx.fillStyle = 'rgba(0, 0, 0, 0.8)'; ctx.strokeStyle = '#f1c40f'; ctx.lineWidth = 2; 
    ...
    if (typeof currentCrates !== 'undefined') {
        currentCrates.forEach(crate => {
            ...
            ctx.save(); ctx.translate(crate.x + crate.size/2, crate.y + crate.size/2);
    ...
    currentDoors.forEach(door => {
        ...
        if (doorImg && doorImg.complete && doorImg.naturalWidth > 0) { 
            ...
            if (currentRoomId === 8 && !worldState.bossDefeated && door.face === 'south') { ctx.fillStyle = 'rgba(192, 57, 43, 0.4)'; ctx.fillRect(door.x, door.y, door.width, door.height); } 
        } 
    });
    ...
    currentEnemies.forEach(enemy => {
        ...
        else if (enemy.type === 'spider') skinName = 'spider_top_view'; else if (enemy.type === 'troll') skinName = 'troll_top_view'; else if (enemy.type === 'mage') skinName = 'mage_top_view'; else if (enemy.type === 'dragon') skinName = 'drake_top_view';
        ...
        if (img && img.complete && img.naturalWidth > 0) {
            ...
            if (enemy.type === 'mage' || enemy.type === 'spider' || enemy.type === 'Burned_top_view') { ctx.save(); ctx.beginPath(); ctx.arc(0, 0, displaySize/2.2, 0, Math.PI*2); ctx.clip(); ctx.drawImage(img, -displaySize/2, -displaySize/2, displaySize, displaySize); ctx.restore(); } 
    ...
    // --- 10. PROJECTILES (Joueurs et Ennemis dont l'OS NET) ---
    ...
    enemyProjectiles.forEach(p => { 
        ...
        if (p.type === 'bone') { 
            ...
            ctx.fillRect(-l, -w, l * 2, w * 2); ctx.beginPath(); ctx.arc(-l, -w*1.2, r, 0, Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(-l, w*1.2, r, 0, Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(l, -w*1.2, r, 0, Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(l, w*1.2, r, 0, Math.PI*2); ctx.fill(); ctx.fillStyle = '#bdc3c7'; ctx.fillRect(-l + 2, -w/4, l*2 - 4, w/2); 
    ...
    // --- 12. PARTICULES (Correction point jaune fixe) ---
