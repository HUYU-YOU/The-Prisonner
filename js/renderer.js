// ============================================================================
// MOTEUR VISUEL ET RENDU (js/renderer.js)
// ============================================================================
...
// MISE À JOUR DU HUD HTML DE L'INVENTAIRE (Y compris les pièces d'or !)
window.updateHUD = function() {
    let healthPercent = (playerStats.health / playerStats.maxHealth) * 100;
    
    let hBar = document.getElementById('health-bar'); 
    if(hBar) hBar.style.width = healthPercent + "%";
    
    let mBar = document.getElementById('mana-bar'); 
    if(mBar) mBar.style.width = playerStats.mana + "%";
    ...
    // NOUVEAU : Affichage des pièces d'or dans le menu HTML !
    let pcoins = document.getElementById('inv-coins'); 
    if(pcoins && playerStats.inventory.coins !== undefined) {
        pcoins.innerText = playerStats.inventory.coins;
    }
}

// LA FONCTION MAÎTRESSE DE DESSIN
window.renderGameView = function() {
    ...
    // --- 2. COULOIRS (Salles 2 et 3) - Murs noirs dynamiques ---
    ...
    // --- SALLE 1 : DÉCORS SPÉCIAUX (BANC ET BIBLIOTHÈQUE) ---
    if (currentRoomId === 1) {
        ...
        if (typeof bookshelf !== 'undefined') {
            ...
            if (imgBiblio && imgBiblio.complete && imgBiblio.naturalWidth > 0) { 
                ...
                ctx.fillStyle = '#2a1a15'; ctx.fillRect(bookshelf.x + 5, bookshelf.y + 30, bookshelf.width - 5, 4); ctx.fillRect(bookshelf.x + 5, bookshelf.y + 70, bookshelf.width - 5, 4); ctx.fillRect(bookshelf.x + 5, bookshelf.y + 110, bookshelf.width - 5, 4); 
                ctx.fillStyle = '#8c1c1c'; ctx.fillRect(bookshelf.x + 10, bookshelf.y + 10, 8, 20); ctx.fillStyle = '#29547d'; ctx.fillRect(bookshelf.x + 20, bookshelf.y + 50, 10, 20); 
            }
            ...
            if (distToBiblio < 120) {
                ctx.save(); 
                ctx.fillStyle = 'rgba(0, 0, 0, 0.8)'; ctx.strokeStyle = '#f1c40f'; ctx.lineWidth = 2; 
                let popW = 320; let popH = 40; let popX = bookshelf.x + bookshelf.width/2 - popW/2; 
                let popY = bookshelf.y - popH - 15; popY += Math.sin(Date.now() / 200) * 3; 
                ctx.fillRect(popX, popY, popW, popH); ctx.strokeRect(popX, popY, popW, popH); 
                ctx.fillStyle = '#ecf0f1'; ctx.font = 'bold 14px Arial'; ctx.textAlign = 'center'; 
    ...
    // --- 4. Rendu des Caisses et Coffres ---
    if (typeof currentCrates !== 'undefined') {
        currentCrates.forEach(crate => {
            ...
            if (!crate.isBroken && crate.health < 30 && crate.type !== 'chest') {
                ctx.rotate(Math.sin(Date.now() / 20) * 0.1); // Secousse si endommagé
            }
    ...
    currentItems.forEach(item => {
        ...
        if (item.type === 'key') { 
            ...
            ctx.fillStyle = '#bdc3c7'; ctx.fillRect(item.x - 3, item.y + 10 + floatY, 6, 15); ctx.fillStyle = '#2c3e50'; ctx.fillRect(item.x - 2, item.y + 2 + floatY, 4, 4); 
    ...
    if (typeof necroSummons !== 'undefined') {
        necroSummons.forEach(s => {
            ...
            if (s.type === 'fusion') { 
                if (s.invulnerableTimer && s.invulnerableTimer > 0) { ctx.shadowBlur = 20; ctx.shadowColor = '#f1c40f'; ctx.strokeStyle = '#f1c40f'; ctx.lineWidth = 4; ctx.beginPath(); ctx.arc(0, 0, s.size/2 + 8, 0, Math.PI*2); ctx.stroke(); } 
                ctx.fillStyle = '#8e44ad'; ctx.fillRect(-s.size/2, -s.size/2, s.size, s.size); ctx.fillStyle = '#1abc9c'; ctx.beginPath(); ctx.arc(0, 0, s.size/4, 0, Math.PI*2); ctx.fill(); 
            } else { 
    ...
    currentEnemies.forEach(enemy => {
        ...
        ctx.rotate(rot); ctx.scale(scalePulse, scalePulse); 
        ...
        if (!img || !img.complete || img.naturalWidth === 0) { 
            let fallbackName = ''; if (enemy.type === 'goblin') fallbackName = 'goblin_top_view'; else if (enemy.type === 'skeleton') fallbackName = 'Skeleton_top_view'; else fallbackName = skinName; 
        ...
        if (enemy.ultiAnimTimer > 0) { 
            ctx.save(); ctx.globalCompositeOperation = 'screen'; let imgUlt = assetsManager.images['Ulti_fire_mage']; 
            if (imgUlt && imgUlt.complete && imgUlt.naturalWidth > 0) { let progress = 1 - (enemy.ultiAnimTimer / 30); let expSize = enemy.size * 2 + (enemy.size * 4 * progress); ctx.globalAlpha = enemy.ultiAnimTimer / 30; ctx.rotate(progress * Math.PI); ctx.drawImage(imgUlt, -expSize/2, -expSize/2, expSize, expSize); } 
            ctx.restore(); 
        }
        ...
        if (!['troll', 'mage', 'dragon'].includes(enemy.type)) { 
            ...
            ctx.fillStyle = '#f1c40f'; ctx.font = 'bold 16px Arial'; ctx.textAlign = 'center'; ctx.fillText(bossName, enemy.x + enemy.size/2, enemy.y - 25); 
            let barWidth = 100; ctx.fillStyle = '#111'; ctx.fillRect(enemy.x + enemy.size/2 - barWidth/2, enemy.y - 15, barWidth, 8); 
    ...
    if (boss && currentRoomId === 8 && !worldState.bossDefeated) {
        ...
        ctx.fillStyle = '#111'; ctx.fillRect(bx, by, barWidth, barHeight); ctx.fillStyle = isPhase2 ? '#8e44ad' : '#e74c3c'; 
        let hpPercent = Math.max(0, boss.health) / boss.maxHealth; ctx.fillRect(bx + 2, by + 2, (barWidth - 4) * hpPercent, barHeight - 4); 
        ctx.fillStyle = isPhase2 ? '#8e44ad' : '#f1c40f'; ctx.font = 'bold 22px Arial'; ctx.textAlign = 'center'; ctx.fillText(bossName + (isPhase2 ? " (ENRAGÉ)" : ""), canvas.width/2, by - 8); ctx.textAlign = 'left';
    }
    ...
    // --- 9. PROJECTILES ENNEMIS (OS NET SANS BRILLANCE) ---
    enemyProjectiles.forEach(p => { 
        ...
        if (p.type === 'bone') { 
            ctx.rotate(pAngle); // Pointe droit devant
            ctx.shadowBlur = 0; // AUCUNE brillance !
            ctx.fillStyle = '#ecf0f1'; // Blanc mat
            
            let l = p.size * 1.5; // Longueur
            let w = p.size * 0.3; // Épaisseur
            let r = p.size * 0.6; // Articulations
    ...
    // --- 11. PARTICULES (Correction disparition) ---
    for (let i = particles.length - 1; i >= 0; i--) {
        let p = particles[i]; 
        p.x += p.vx; p.y += p.vy; 
        p.life -= 0.05; // FIX : Enlève les points jaunes résiduels
        if (p.life <= 0) {
    ...
    // --- 12. VIGNETTE DE LUMIÈRE (BROUILLARD) ---
    ctx.save();
