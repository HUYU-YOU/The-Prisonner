...
function renderGameView() {
  ...
  ctx.strokeStyle = currentRoomId === 999 ? '#222' : '#3d342c';
  ctx.lineWidth = 1;
  ...
  // HISTOIRE / TUTORIEL OBETS DE SALLE 1
  if (currentRoomId === 1) {
  ...
  if (drawPlayer) {
    ...
    ctx.fillStyle = playerPoisonTimer > 0 ? '#27ae60' : 
                   (player.heroClass === 'Elf' ? '#2ecc71' : 
                   (player.heroClass === 'Mage' ? '#e67e22' : '#95a5a6'));
    ...
    if (player.heroClass === 'Elf') {
      ...
      if(attackCooldown <= 0) { ctx.fillStyle = '#ecf0f1'; ctx.fillRect(10, -1, 15, 2); }
      ...
      ctx.fillStyle = '#d35400'; ctx.beginPath(); ctx.arc(0, 0, 15, 0, Math.PI*2); ctx.fill();
      ctx.strokeStyle = '#f1c40f'; ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(10, 0, 10, -Math.PI/2, Math.PI/2); ctx.stroke();
    }
  ...
  // MESSAGES OVERLAYS ET TEXTES DE SCENARIO
  if (currentRoomId === 1) {
    let distToBookshelf = Math.hypot((player.x + player.size/2) - (bookshelf.x + bookshelf.width/2), (player.y + player.size/2) - (bookshelf.y + bookshelf.height/2));
    if (distToBookshelf < 100) {
      ctx.fillStyle = 'rgba(0, 0, 0, 0.8)'; ctx.fillRect(canvas.width / 2 - 350, canvas.height - 120, 700, 60);
      ctx.fillStyle = '#e74c3c'; ctx.font = '22px Courier New'; ctx.textAlign = 'center';
      ctx.fillText(`"LA PRISON N'EST PAS UNE PRISON, LE DONJON L'EST."`, canvas.width / 2, canvas.height - 85);
    }
  }
  
  if (currentRoomId === 8) {
      ctx.fillStyle = 'rgba(0, 0, 0, 0.9)'; ctx.fillRect(canvas.width / 2 - 300, canvas.height / 2 - 100, 600, 200);
      ctx.fillStyle = '#e74c3c'; ctx.font = 'bold 40px Courier New'; ctx.textAlign = 'center';
      ctx.fillText("NIVEAU 1 TERMINÉ", canvas.width / 2, canvas.height / 2);
      ctx.fillStyle = '#bdc3c7'; ctx.font = '20px Courier New';
      ctx.fillText("Les monstres vous attendent plus bas...", canvas.width / 2, canvas.height / 2 + 50);
  }

  if (currentRoomId === 999) {
    ctx.fillStyle = 'rgba(0, 0, 0, 0.8)'; ctx.fillRect(canvas.width / 2 - 200, 10, 400, 50);
    ctx.fillStyle = '#ecf0f1'; ctx.font = '20px Courier New'; ctx.textAlign = 'center';
    if (arenaState === "WAITING") {
      let secondsLeft = Math.ceil(arenaTimer / 60); ctx.fillText(`VAGUE ${arenaWave} IMMINENTE... (${secondsLeft}S)`, canvas.width / 2, 42);
    } else { ctx.fillText(`VAGUE ${arenaWave - 1} | ABERRATIONS RESTANTES : ${currentEnemies.length}`, canvas.width / 2, 42); }
    ...
    if (boss) {
        ...
        let bName = boss.type === 'troll' ? "LE TROLL CORROMPU" : (boss.type === 'mage' ? "LE MAGE SOMBRE" : "LE DRAGON ANCIEN");
        if(boss.phase === 2) bName += " - SURVIVEZ !";
        ctx.fillText(bName, canvas.width / 2, 90);
    }
  }
